    ...
    def __init__(self,
        ...
        for layer in range(num_layers):
            for direction in range(num_directions):
                ...
                w_ih = nn.Parameter(torch.Tensor(gate_size, layer_input_size // groups, *self.kernel_size))
                w_hh = nn.Parameter(torch.Tensor(gate_size, out_channels // groups, *self.kernel_size))

                b_ih = nn.Parameter(torch.Tensor(gate_size))
                b_hh = nn.Parameter(torch.Tensor(gate_size))

                if mode == 'PeepholeLSTM':
                    w_pi = nn.Parameter(torch.Tensor(out_channels, out_channels // groups, *self.kernel_size))
                    w_pf = nn.Parameter(torch.Tensor(out_channels, out_channels // groups, *self.kernel_size))
                    w_po = nn.Parameter(torch.Tensor(out_channels, out_channels // groups, *self.kernel_size))
